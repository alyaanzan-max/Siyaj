...
# --- 🎓 3. الأكاديمية الرقمية (النسخة النهائية المعتمدة) ---
    ...
    with col1:
        ...
        if st.button("📖 شرح دروس المسار الأساسي"):
            st.info("عد لغة بايثون الخيار الاستراتيجي لتطوير "منظومة سياج"، وذلك لعدة أسباب تقنية:

الكفاءة في الأمن السيبراني: بايثون هي اللغة الرائدة عالمياً في تطوير أدوات الحماية والبرمجيات الأمنية نظراً لتوفر مكتبات متقدمة تدعم التشفير وتحليل البيانات.

سهولة التطوير والقراءة: بساطة بناء الكود في بايثون تسمح للمبرمج بالتركيز على المنطق البرمجي وحل المشكلات المعقدة بدلاً من الانشغال بتعقيدات اللغة.

تكامل الأنظمة: مكنتنا بايثون من ربط واجهة الموقع (Streamlit) بخدمات خارجية وبرمجيات معقدة بسلاسة تامة.

أساسيات علوم التشفير (Cryptography)
تم بناء "مختبر التشفير" في سياج بناءً على مفاهيم علمية دقيقة تهدف لحماية سرية البيانات:

ماهية التشفير: هو عملية تحويل "النص الواضح" (Plaintext) الذي يمكن قراءته، إلى "نص مشفر" (Ciphertext) وهو عبارة عن رموز غير مفهومة، وذلك باستخدام "خوارزميات" (Algorithms) رياضية معقدة.

أركان العملية الأمنية:

الخوارزمية: هي الطريقة البرمجية المتبعة لتغيير شكل البيانات.

المفتاح (Key): هو العنصر الحاسم في التشفير؛ فبدون المفتاح الصحيح، يستحيل استعادة البيانات الأصلية حتى لو تمت سرقتها.

أنواع التشفير المستخدمة:

التشفير المتماثل (Symmetric): وهو استخدام مفتاح واحد لعمليتي التشفير وفك التشفير (مثل خوارزمية AES)، ويتميز بالسرعة العالية في حماية البيانات الضخمة.

التشفير غير المتماثل (Asymmetric): ويعتمد على زوج من المفاتيح (مفتاح عام للتشفير ومفتاح خاص لفك التشفير)، وهو ما يضمن أعلى مستويات الأمان في الاتصالات الرقمية.

الهدف النهائي: نهدف من خلال هذه التقنيات إلى تحقيق مبدأ "السرية" (Confidentiality)، بحيث تظل البيانات محمية وغير قابلة للاختراق حتى في حال وصول أطراف غير مصرح لها إلى قاعدة البيانات.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.button("📖 شرح دروس المسار العملي"):
            st.success("بناء جدار الحماية (Firewall Construction)
الجدار ليس مجرد "حاجز"، بل هو نظام فحص ذكي.

الجانب النظري: يعمل جدار الحماية كمرشح (Filter) لحركة المرور في الشبكة. يقوم بفحص كل "حزمة بيانات" (Packet) تدخل أو تخرج بناءً على قواعد أمنية محددة (Security Rules).

التطبيق في سياج: يمكننا برمجة جدار الحماية بلغة بايثون لمراقبة العناوين (IP Addresses) المشبوهة، فإذا حاول عنوان محظور الوصول للمنصة، يتم "إسقاط" الطلب فوراً.

2. تحليل البرمجيات الخبيثة (Malware Analysis)
وهو العلم الذي يدرس سلوك الملفات المشبوهة لفهم ضررها.

التحليل الساكن (Static Analysis): فحص الكود البرمجي للملف بدون تشغيله (مثل قراءة السورس كود) للبحث عن دوال مشبوهة.

التحليل الديناميكي (Dynamic Analysis): تشغيل الملف في بيئة معزولة وآمنة تسمى (Sandbox) لمراقبة سلوكه: هل يحاول سرقة ملفات؟ هل يتصل بسيرفر خارجي؟

التطبيق في سياج: في "ركن الابتكار"، نوفر بيئة محاكاة تتيح للمستخدم رفع ملف مشبوه، وتقوم المنظمة بفحصه تقنياً وإعطاء تقرير عن مدى خطورته.)")
        st.markdown("</div>", unsafe_allow_html=True)
...
# --- 📡 5. مشوش التنصت (مع شرح سند) ---
    ...
    <div class='feature-card'>
        ...
        <p><b>شرح سند:</b> اسمعي يا علو، المشوش هذا مهوب مجرد لعبة. هو يستخدم تقنية (Quantum Noise Injection). لما تفعلينه، سياج يرسل بيانات وهمية مشفرة في نفس قناة الاتصال، فإذا فيه أحد يتنصت، ما راح يلقى إلا "هذرة" رقمية ما لها معنى. وبكذا، صوتك وبياناتك تكون في أمان خلف جدار من الضجيج المنظم!</p>
    </div>
    """, unsafe_allow_html=True)
