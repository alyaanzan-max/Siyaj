...
if section == "الرئيسية 🏠":
    ...
    with t2:
        st.subheader("📡 رادار الرصد الاستباقي (محاكاة جغرافية)")
        # إضافة الخريطة (الإضافة الجديدة)
        map_data = pd.DataFrame(
            np.random.randn(10, 2) / [15, 15] + [24.7136, 46.6753], 
            columns=['lat', 'lon'])
        st.pydeck_chart(pdk.Deck(
            map_style='mapbox://styles/mapbox/dark-v9',
            initial_view_state=pdk.ViewState(latitude=24.71, longitude=46.67, zoom=4, pitch=50),
            layers=[pdk.Layer('ScatterplotLayer', data=map_data, get_position='[lon, lat]', get_color='[200, 30, 0, 160]', get_radius=50000)]
        ))
    ...
    with t4:
        ...
        idea = st.text_area("أكتبي ابتكارك هنا (سياج كالأم ستتبنى فكرتك):", placeholder="مثلاً: إضافة فاحص للرسائل نصية...")
        if st.button("إرسال الابتكار للمختبر"):
            st.success("تم استلام فكرتك يا مبدعة! سياج ستدرسها وتطورها لتكون جزءاً من الدرع الحامي شكرًا لك.")
