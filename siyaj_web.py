...
# وظيفة إرسال التنبيهات للتليجرام (إضافة تقنية)
    ...
    with st.container():
        ...
        if st.button("تأكيد الهوية الرقمية وفتح التشفير 🔓"):
            if u_name and u_email_input and gate_code == SAFE_CODE:
                ...
                # إرسال تنبيه تسجيل دخول للبوت (إضافة)
...
if section == "الرئيسية 🏠":
    ...
    st.title("📋( سجل المراقبة والإدارة")
    ...
    if st.button("إرسال البلاغ المشفر 🚨"):
        if report_text:
            # إرسال بلاغ للبوت (إضافة)
            send_telegram_notification(f"🚨 بلاغ طوارئ جديد!\nالمبلغ: {current_user}\nالتفاصيل: {report_text}")
            st.balloons()
            st.success(f"شكراً يا {current_user}.. تم رفع بلاغك وتأمينه بنجاح تحت حماية سياج. اطمئني، سند معك!")
        else:
