...
# --- ❓ 9. الدليل التشغيلي (شرح المنظومة بلمسة علو) ---
    ...
    st.write("يا علو، هنا مساحتك الخاصة.. اشرحي لكل قسم وش يسوي بالضبط:")

    # 1. قسم العمليات المركزية
    with st.expander("📖 شرح قسم 🏠 العمليات المركزية:"):
        st.write("هذا القسم يمثل مركز الإدارة والتحكم (Dashboard). تقنياً، هو الشاشة التي تجمع كافة البيانات من الأقسام الأخرى لتعرضها بشكل مركزي. وظيفته هي "المراقبة اللحظية"، حيث يتم رصد حالة النظام، وتدفق البيانات، والتأكد من أن جميع الدروع الرقمية تعمل بكفاءة. هو باختصار "نقطة المرجعية" لأي قرار أمني يتم اتخاذه داخل المنصة")

    # 2. قسم ركن الابتكار
    with st.expander("📖 شرح قسم 💡 ركن الابتكار:"):
        st.write("هو بمثابة مختبر التطوير (R&D Lab)، ويحتوي على ثلاث ركائز تقنية:

التوثيق التاريخي: قصة "سياج" التي تشرح كيف بدأ المشروع.

التكامل البرمجي (Integration): وهو صندوق الأفكار المربوط تقنياً عبر (API) ببوت التليجرام الخاص بك، مما يسمح بنقل البيانات من بيئة الويب إلى بيئة التواصل الفوري.

بيئة المحاكاة (Simulation Environment): كود برمجي ينفذ سيناريوهات افتراضية للهجمات لقياس سرعة استجابة النظام.")

    # 3. قسم الأكاديمية الرقمية
    with st.expander("📖 شرح قسم 🎓 الأكاديمية الرقمية:"):
        st.write("هذا القسم متخصص في التوعية الأمنية (Security Awareness). من الناحية التقنية، الأمن السيبراني لا يعتمد فقط على البرمجيات، بل على "العنصر البشري". وظيفته هي تدريب المستخدمين على أسس الدفاع الرقمي، مثل كيفية تمويل الروابط الاحتيالية وحماية البيانات الشخصية، ليكون المستخدم نفسه "جدار حماية" بشري.")

    # 4. قسم جواز سياج
    with st.expander("📖 شرح قسم 🎫 جواز سياج:"):
        st.write("هو نظام إدارة الهوية والوصول (IAM - Identity and Access Management). تقنياً، يعمل كبوابة للتحقق من هوية المستخدم وصلاحياته. يضمن هذا القسم أن كل شخص يدخل إلى النظام لديه "تصريح" محدد، مما يمنع الدخول غير المصرح به (Unauthorized Access) للأجزاء الحساسة من المنصة.")

    # 5. قسم مشوش التنصت
    with st.expander("📖 شرح قسم 📡 مشوش التنصت:"):
        st.write("يرمز هذا القسم إلى تقنيات تأمين قنوات الاتصال. وظيفته منع عمليات "اعتراض البيانات" (Sniffing) التي قد يقوم بها المخترقون للتنصت على ما يتم إرساله. تقنياً، يعتمد على عزل البيانات ومنع أي طرف ثالث من استراق السمع أو سحب المعلومات أثناء انتقالها بين المرسل والمستقبل.")

    # 6. قسم مختبر التشفير
    with st.expander("📖 شرح قسم 🔑 مختبر التشفير:"):
        st.write("هذا هو قلب علم التعمية (Cryptography) في المشروع. وظيفته تحويل البيانات من نص مفهوم إلى رموز معقدة باستخدام خوارزميات برمجية (مثل AES). الهدف التقني منه هو حماية "سرية البيانات"؛ بحيث لو استطاع أحد الوصول إلى البيانات، فلن يتمكن من قراءتها أو فهمها بدون امتلاك "مفتاح فك التشفير" الخاص بسياج.")

    # 7. قسم بلاغ الطوارئ
    with st.expander("📖 شرح قسم 🚨 بلاغ الطوارئ:"):
        st.write("يمثل نظام الاستجابة الفورية للحوادث (Incident Response). هو قناة اتصال ذات أولوية قصوى، وظيفتها تقليص "زمن الاكتشاف" و"زمن الاستجابة". تقنياً، يوفر واجهة سريعة لإرسال تنبيهات فورية عند وقوع أي نشاط مشبوه، مما يسمح للنظام أو المشرفين بالتدخل السريع لإيقاف الهجوم قبل تفاقمه.")
